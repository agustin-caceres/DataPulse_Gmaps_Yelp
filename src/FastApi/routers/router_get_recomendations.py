...
async def get_recomendations(
        request_app: BaseRecommendationRequest) -> BaseRecomendations:
    """
    get_recomendations: recibe una peticion GET con payload con la estructura 
                        de BaseRecommendationRequest y devuelve las n
                        recomendaciones solicitadas con la estructura
                        BaseRecomendations
    ...
    try:
        ...
        mask = df_user_ids['user_id'] == request_app.user_id_string
        ...
        scores = model.predict(
            user_id, business_id_list, item_features=item_features, user_features=user_features)
